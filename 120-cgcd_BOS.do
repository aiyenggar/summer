import delimited "/Users/aiyenggar/OneDrive/stata/qgis/cg_rawassignee_rawlocation_cluster_BOS.csv", encoding(ISO-8859-1)clear
rename * cga_*
rename cga_patent_id patent_id
save cg_rawassignee_rawlocation_cluster.dta, replace

import delimited "/Users/aiyenggar/OneDrive/stata/qgis/cgcd_uspatentcitation_BOS.csv", encoding(ISO-8859-1)clear
rename * cit_*
...
merge m:m cd_patent_id using cd_rawinventor_rawassignee_BOS.dta, gen(_mcd)

export delimited using "/Users/aiyenggar/OneDrive/stata/qgis/cgcd_BOS.csv", replace
...
export delimited using "/Users/aiyenggar/OneDrive/stata/qgis/cgcd_minimal_BOS.csv", replace
save cgcd_minimal_BOS.dta, replace
...
graph twoway (scatter la cit_year) (scatter lap cit_year) ///
	...
	title("Knowledge Flows from Boston") ///
...
graph save Boston.gph, replace

drop if cit_year < 1990 | cit_year > 2015
graph twoway (scatter la cit_year) (scatter lap cit_year) ///
	(scatter lpa cit_year) (scatter lpap cit_year), ///
	ytitle("Number of Patent Citations") xtitle("Year of Citation") ///
	title("Knowledge Flows from Boston since 1990") ///
	note("Source: PatentsView.org") ///
	legend(label(1 Local Firm) label(2 Cluster) label(3 Geographical Diversification) label(4 Diffusion))
graph save Boston1990.gph, replace
